...
Version
c: simplify classes, don't worry about reuse; try bsp again
d: Add the rooms
...
class Dungeon:
    ...
    def makePathB(self, ra, rb): # path btwn room ra and room rb, return xy pts
        x = ra.left + int(ra.width/2)
        y = ra.top + int(ra.height/2)
        xend = rb.left + int(rb.width/2)
        yend = rb.top + int(rb.height/2)
        #print ("(",x, y, ") -> (",xend, yend, ") ===")
        path = [(x,y)]
    ...
    def carve(self, room):
        w = room.width
        h = room.height
        room.width = random.randint(3, room.width-3)
        room.height = random.randint(3, room.height-3)
        room.left = room.left + int((w-room.width)/2)
        room.top = room.top + int((h-room.height)/2)
        return room
    ...
    def build(self):
        ...
        print ("="*20)
        for r in self.rooms:
            print (n, r.left, r.top, r.right, r.bottom)
            n += 1
        """

        for n in range(len(self.rooms)):
            self.rooms[n] = self.carve(self.rooms[n])
        ...
        while lvl <= len(self.rooms):
            for n in range(0, len(self.rooms), lvl):
                self.paths.append(self.makePathB(self.rooms[n], self.rooms[n+lvl-1]))
            lvl *= 2
        """
        """
